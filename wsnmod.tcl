 ...
 set val(nam) wsnmod.nam
 set val(traffic) cbr                        ;# cbr/poisson/ftp
 
 exec awk modification.awk &
 ...
 set tracefd     [open ./wsnmod.tr w]
 $ns_ trace-all $tracefd
 if { "$val(nam)" == "wsnmod.nam" } {
 ...
 proc cbrtraffic { src dst interval starttime } {
	...
	for { set a 0}  {$a < 30} {incr a 3} {
	   $ns_ at $a "exec awk -f Scripts/resenergy.awk wsnmod.tr > Modified/${a}_ResidualEnergy &"
	}
 ...
 if { ("$val(traffic)" == "cbr") || ("$val(traffic)" == "poisson") } {
    ...
    eval $val(traffic)traffic 19 6 0.20 $appTime1
    eval $val(traffic)traffic 10 4 0.20 $appTime2
    eval $val(traffic)traffic 3 2 0.19 $appTime3
    ...
    $ns_ at 0.08 "$ns_ trace-annotate \"$val(traffic) traffic from node 19 to node 6: 19 20 21 24 6\""
    ...
    $ns_ at 0.35 "$ns_ trace-annotate \"$val(traffic) traffic from node 10 to node 4: 10 13 15 4\""
    ...
    $ns_ at 0.78 "$ns_ trace-annotate \"$val(traffic) traffic from node 3 to node 2: 3 16 7 2\""
 }
 ...
 if { "$val(traffic)" == "ftp" } {
    ...
    $ns_ at [expr $appTime1+0.09] "$ns_ trace-annotate \"ftp traffic from node 19 to node 6: 19 20 21 24 6\""
    $ns_ at $appTime2 "$node_(10) NodeClr green4"
    $ns_ at $appTime2 "$node_(4) NodeClr green4"
    $ns_ at $appTime2 "$ns_ trace-annotate \"(at $appTime2) ftp traffic from node 10 to node 4\""
    $ns_ at [expr $appTime2+0.09] "$ns_ trace-annotate \"ftp traffic from node 10 to node 4: 10 13 15 4\""
    $ns_ at $appTime3 "$node_(3) NodeClr cyan3"
    $ns_ at $appTime3 "$node_(2) NodeClr cyan3"
    $ns_ at $appTime3 "$ns_ trace-annotate \"(at $appTime3) ftp traffic from node 3 to node 2\""
    $ns_ at [expr $appTime3+0.09] "$ns_ trace-annotate \"ftp traffic from node 3 to node 2: 3 16 14 2\""
 }
 ...
 proc stop {} {
     ...
     if { ("$val(nam)" == "wsnmod.nam") && ("$hasDISPLAY" == "1") } {
     exec nam wsnmod.nam &

     exec awk -f Scripts/resenergy.awk wsnmod.tr > Modified/ResidualEnergy &
     exec awk -f Scripts/Throughput.awk wsnmod.tr > Modified/AvgThroughput &
     exec awk -f Scripts/pdr.awk wsnmod.tr > Modified/PDR &
     }
 }

 ...
 set tracefd     [open ./wsnunmod.tr w]
 $ns_ trace-all $tracefd
 if { "$val(nam)" == "wsnunmod.nam" } {
 ...
 proc cbrtraffic { src dst interval starttime } {
	...
	for { set a 0}  {$a < 30} {incr a 3} {
	   $ns_ at $a "exec awk -f Scripts/resenergy.awk wsnunmod.tr > Unmodified/${a}_ResidualEnergy &"
	}
 ...
 proc stop {} {
     ...
     if { ("$val(nam)" == "wsnunmod.nam") && ("$hasDISPLAY" == "1") } {
     exec nam wsnunmod.nam &

     exec awk -f Scripts/resenergy.awk wsnunmod.tr > Unmodified/ResidualEnergy &
     exec awk -f Scripts/Throughput.awk wsnunmod.tr > Unmodified/AvgThroughput &
     exec awk -f Scripts/pdr.awk wsnunmod.tr > Unmodified/PDR &
     }
 }
